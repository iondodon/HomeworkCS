...
class DSA:
    def __init__(self):
        # N = 160
        # L = 1024
        # p, q, g = self.generate_params(L, N)
        # x, y = self.generate_keys(g, p, q)
        #
        # text = "MISIS rocks"
        # M = str.encode(text, "ascii")
        # r, s = self.sign(M, p, q, g, x)
        # if self.verify(M, r, s, p, q, g, y):
    ...
    def sign(self, M, p, q, g, x):
        if not self.validate_params(p, q, g):
    ...
    def verify(self, M, r, s, p, q, g, y):
        if not self.validate_params(p, q, g):
            raise Exception("Invalid params")
        if not self.validate_sign(r, s, q):
            return False
        try:
            w = invert(s, q)
        except ZeroDivisionError:
